  ...
  <div>
    ...
    <header class="sticky top-0 z-10 flex items-center justify-between border-b bg-white px-3 py-2.5 sm:px-5">
      <div class="flex items-center space-x-2">
        ...
        <!-- Manually Displayed Material Label -->
        <router-link :to="{ name: 'Material' }" class="text-gray-600 hover:text-black">
          {{ __('Material') }}
        </router-link>
      </div>

      <div class="flex space-x-2 justify-end">
        <router-link :to="{ name: 'MaterialDemand', params: { courseName: 'new' } }" class="inline-block">
	  ...
	  <div class="mt-4">
		...
		<div class="p-4">
			...
			<div v-if="currentSeries.length > 0">
				...
				@click="['Released', 'Locked'].includes(series.document_status) ? fetchDetails(series.name) : null"
				>
...
<section class="mt-4">
  <div class="flex flex-col sm:flex-row justify-start items-center space-y-2 sm:space-y-0">
	...
	<ModalWrapper :isOpen="isModalOpen" title="demand Details" @close="isModalOpen = false">
		<div class="max-w-full sm:max-w-lg md:max-w-xl lg:max-w-2xl xl:max-w-3xl w-full h-auto sm:h-96 md:h-104 lg:h-128 xl:h-144">
		<DemandForm :demand="selectedDemand" @update="handleUpdate" @close="isModalOpen = false" />
		</div>
	</ModalWrapper>

	<!-- Modal for Adding New Requirement -->
	<ModalWrapper :isOpen="isAddDemandModalOpen" title="Add New Demand" @close="closeAddDemandModal">
	<div class="max-w-full sm:max-w-lg md:max-w-xl lg:max-w-2xl xl:max-w-3xl w-full h-auto sm:h-96 md:h-104 lg:h-128 xl:h-144">
		<DemandForm :demand="newDemand" @update="handleAddDemand" @close="closeAddDemandModal" />
	</div>
	</ModalWrapper>

    <div v-if="updateSuccess" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
          <h3 class="text-xl font-semibold text-green-800 mb-4">{{ updateSuccess }}</h3>
        </div>
      </div>
	</div>
  </template>
  ...
  import DemandForm from './MaterialDemandForm.vue';
  
  const seriesNumbers = ref([]);
  const selectedDemand = ref(null);
  const newDemand = ref({});
  const isModalOpen = ref(false);
  const isAddDemandModalOpen = ref(false);
...
const fetchSeriesNumbers = async () => {
  try {
    ...
    if (!loggedInUser.value) {
      console.error("No logged-in user found. Unable to fetch Demand.");
      return;
    }

    // Fetch requirements belonging to the logged-in user
    const response = await axios.get('/api/resource/Material Demand', {
      params: {
  ...
  const fetchSeriesDetails = async (series) => {
	try {
	  const response = await axios.get(`/api/resource/Material Demand/${series.name}`);
  ...
  const fetchDetails = async (seriesNumber) => {
	try {
	  const response = await axios.get(`/api/resource/Material Demand/${seriesNumber}`);
	  selectedDemand.value = response.data.data || {};
  ...
  const handleUpdate = async (updatedDemand) => {
  try {
    await axios.put(`/api/resource/Material Demand/${updatedDemand.name}`, updatedDemand);
    const index = seriesNumbers.value.findIndex(series => series.name === updatedDemand.name);
    if (index !== -1) {
    ...
    updateSuccess.value = 'Update was successful!'; // Set success message
    setTimeout(() => updateSuccess.value = null, 1000); // Remove message after 5 seconds
  } catch (error) {
    console.error('Error updating Demand:', error.response?.data?.message || error.message || 'Unknown error');
    updateSuccess.value = 'Failed to update demand, please try again.'; // Set error message if needed
    setTimeout(() => updateSuccess.value = null, 1000); // Remove message after 5 seconds
  }
  ...
  const handleAddDemand = async (newDemand) => {
	try {
	  await axios.post('/api/resource/Material Demand', newDemand);
	  fetchSeriesNumbers(); 
	  isAddDemandModalOpen.value = false; 
	} catch (error) {
	  console.error('Error adding Demand:', error.response?.data?.message || error.message || 'Unknown error');
	}
  };

  
  // Close Add Requirement Modal 
  const closeAddDemandModal = () => {
	isAddDemandModalOpen.value = false;
  };
  ...
  const openAddDemandModal = () => {
	isAddDemandModalOpen.value = true;
  };
