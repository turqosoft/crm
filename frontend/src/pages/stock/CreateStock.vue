<template>
      ...
      <div class="w-full max-w-4xl mx-auto py-5">
        ...
        <div class="mb-4">
          ...
          <div v-if="currentTab === 'itemDetails'" class="mt-4">
            <div v-for="(item, index) in itemDetails" :key="index" class="p-4 border rounded-lg shadow-sm cursor-pointer" @click="openPopup(item)">
  ...
  import { Breadcrumbs, FormControl, Button, createResource } from 'frappe-ui'
  ...
  // Resource for fetching current_qty 
  const binFieldsResource = createResource({
    url: 'tqerp_horticorp.tqerp_horticorp.utils.get_bin_fields_and_current_qty',
    auto: false,
  })
  ...
  const openPopup = async (item) => {
    ...
    if (existingItem) {
      ...
      popupData.wastage_qty = 0; // Reset wastage quantity if the item is new
    }
