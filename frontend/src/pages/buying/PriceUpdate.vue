<template>
  ...
  <div class="flex items-center space-x-2">
    <span v-for="(item, index) in items" :key="index" class="text-sm text-gray-700">
      <span>{{ item.name }}</span>
...
const addRate = () => {
  ...
  // Check if the rate is negative or zero
  if (price_list_rate.value <= 0) {
    showToast('Rate cannot be negative or zero.', 'warning');
    return;
  }

  // Proceed with the logic to add the rate if the validation passes
  // (Your logic to add the rate goes here)
